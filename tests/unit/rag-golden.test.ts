...
describe('Horizon RAG golden set v1', () => {
  it('contains at least 200 unique, sanitized, versioned requests', () => {
    expect(HORIZON_RAG_GOLDEN_V1.length).toBeGreaterThanOrEqual(200);
    expect(new Set(HORIZON_RAG_GOLDEN_V1.map((item) => item.id)).size).toBe(
      HORIZON_RAG_GOLDEN_V1.length,
    );
    expect(new Set(HORIZON_RAG_GOLDEN_V1.map((item) => item.request)).size).toBe(
      HORIZON_RAG_GOLDEN_V1.length,
    );
  ...
  it('meets the 98% deterministic routing promotion gate', () => {
    const correct = HORIZON_RAG_GOLDEN_V1.filter(
      (item) => routeRetrievalIntent(item.request).route === item.expectedRoute,
    ).length;
