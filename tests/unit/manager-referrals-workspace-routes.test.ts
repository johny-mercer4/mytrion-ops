...
describe('Manager referral workspace route', () => {
  it('requires authentication before touching Zoho or MART', async () => {
    const response = await app.inject({ method: 'GET', url: '/v1/manager/referrals/workspace' });
    expect(response.statusCode).toBe(401);
    expect(workspaceMock).not.toHaveBeenCalled();
  });

  it('rejects workers outside Management', async () => {
    const response = await app.inject({
      method: 'GET',
      url: '/v1/manager/referrals/workspace',
  ...
  it('passes an explicit calendar month and tenant context to the workspace', async () => {
    const response = await app.inject({
      method: 'GET',
      url: '/v1/manager/referrals/workspace?period_month=2026-06-01',
  ...
  it('rejects a partial month before any source read', async () => {
    const response = await app.inject({
      method: 'GET',
      url: '/v1/manager/referrals/workspace?period_month=2026-06-15',
      headers: await bearer('Management'),
    });
