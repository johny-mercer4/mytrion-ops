/**
 * Zoho OAuth primitives (shared by the app's auth Wrapper and the metadata analyzers).
 *
 * One self-client app (ZOHO_CLIENT_ID/SECRET) backs all Zoho services, but each service
 * needs a refresh token minted with that service's scopes — so tokens resolve per service,
 * falling back to a shared ZOHO_REFRESH_TOKEN when present.
 *
...
export interface ZohoToken {
  ...
  /** Token lifetime in seconds (Zoho default ~3600). Used by the Wrapper for caching. */
  expiresInSec: number;
}
...
export function resolveZohoConfig(service: ZohoService): ZohoServiceConfig {
  ...
  if (missing.length > 0) {
    throw new Error(`[zoho:${service}] missing env: ${missing.join(', ')}`);
  }
