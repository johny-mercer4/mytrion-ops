 ...
 * long OpenAI worked, and exactly how many tokens it cost (input / output / cached input,
 * straight from the API usage object).
 *
...
let recent: TurnLog[] = [];
let lastMtimeMs = -1;
const turnWriter = createBufferedJsonlWriter();
...
function syncFromFile(): void {
  ...
  try {
    ...
    if (turnWriter.pending() > 0) return;
...
function aggregates() {
  ...
  return {
    ...
    waitMaxMs: percentile(waits, 100),
    totalP95Ms: percentile(totals, 95),
    inTok: inputTokens,
    outTok: sum(day, (r) => r.outTok),
    cacheRead,
    cacheWrite: sum(day, (r) => r.cacheWrite),
    // cached_tokens is a subset of input_tokens, not an additional token count.
    cacheHitPct: inputTokens > 0 ? Math.round((100 * cacheRead) / inputTokens) : 0,
  };
}
