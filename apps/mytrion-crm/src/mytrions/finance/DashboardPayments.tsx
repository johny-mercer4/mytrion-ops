    ...
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {sourceSummaries.map((s) => (
          <div key={s.src} className="flex items-center gap-3 rounded-lg border bg-card p-4 shadow-sm">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1.5">
                ...
                <span className="rounded-md bg-secondary px-1.5 py-0.5 text-[10px] font-bold text-secondary-foreground">{s.count}</span>
              </div>
          ...
          {DASHBOARD_PAYMENTS.map((p, i) => {
            ...
            return (
              <div key={i} className="grid grid-cols-[1fr_1fr_2fr_1fr_1fr] items-center gap-2.5 border-b px-4 py-3 text-sm last:border-b-0">
                <span className="w-fit rounded-md bg-secondary px-1.5 py-0.5 font-mono text-[10px] font-extrabold tracking-wide text-secondary-foreground uppercase">
                  {p.src}
                </span>
                ...
                <span>
                  <span
                    className={`rounded-md border px-2 py-0.5 text-[10px] font-semibold ${declined ? 'border-bad/30 bg-bad/10 text-bad' : 'border-good/30 bg-good/10 text-good'}`}
                  >
                    {p.st}
                  </span>
                </span>
