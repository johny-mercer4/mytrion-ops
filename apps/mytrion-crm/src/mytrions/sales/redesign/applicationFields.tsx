 ...
 * styles + `ss-*` classes), not `ds/` — the whole redesign tree is built that way and mixing the two
 * inside one form is what makes a screen look assembled rather than designed.
 *
...
import { FIELD, LABEL } from './createTicketShared';
...
export function Section({
  ...
  return (
    <section style={s('display:grid;gap:14px')}>
      <div>
        <h3 style={s('margin:0;font-size:15px;font-weight:800;color:var(--text)')}>{title}</h3>
        {hint ? (
          <p style={s('margin:4px 0 0;font-size:13px;color:var(--muted);line-height:1.5')}>{hint}</p>
        ) : null}
      </div>
  ...
  return (
    <div style={s('display:flex;flex-direction:column')}>
      ...
      <input
        ...
        // 16px on mobile or iOS zooms the viewport on focus; the shared FIELD constant already
        // sets 14px, so the guard is the `--text-input-mobile` token applied in CSS, not here.
      ...
      />
      {hint ? (
        <span style={s('margin-top:6px;font-size:12px;color:var(--faint);line-height:1.45')}>{hint}</span>
      ) : null}
    </div>
  );
}
    ...
    >
      {options.map((o) => {
        ...
        return (
          ...
          >
            <span style={s('display:flex;align-items:center;gap:8px;font-size:14px;font-weight:800;color:var(--text)')}>
              {active ? <Icon name="check" size={15} color="var(--accent)" strokeWidth={2.4} /> : null}
              {o.title}
            </span>
            <span style={s('font-size:12px;color:var(--muted);line-height:1.5')}>{o.body}</span>
          </button>
        );
      })}
    </div>
...
export function GateBanner({
  ...
  if (submitted && complete) {
    return (
      <div
        role="status"
        style={s(
          'display:flex;align-items:center;gap:10px;padding:12px 14px;border-radius:var(--radius-md);background:var(--intent-success-bg,rgba(52,211,153,.1));border:1px solid var(--intent-success-bd,rgba(52,211,153,.3))',
        )}
      >
        <Icon name="check" size={18} color="var(--ok)" strokeWidth={2.4} />
  ...
  return (
    <div
      role="status"
      style={s(
        'display:grid;gap:8px;padding:12px 14px;border-radius:var(--radius-md);background:var(--intent-danger-bg,rgba(248,113,113,.1));border:1px solid var(--intent-danger-bd,rgba(248,113,113,.32))',
      )}
    >
      ...
      <ul style={s('margin:0;padding-left:28px;display:grid;gap:3px')}>
        {missing.slice(0, 6).map((m) => (
          <li key={m.field} style={s('font-size:12px;color:var(--text2);line-height:1.5')}>
            {m.label}
          </li>
        ))}
        {missing.length > 6 ? (
          <li style={s('font-size:12px;color:var(--faint)')}>and {missing.length - 6} more…</li>
        ) : null}
      </ul>
    </div>
  );
