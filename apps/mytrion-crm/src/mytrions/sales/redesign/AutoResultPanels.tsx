...
export function AutoTransactionsPanel({
  ...
  const liveTotals = useMemo(
    () => ({
      ...
      funded: processed.reduce((s, t) => s + (t.fundedTotal || 0), 0),
      discount: processed.reduce((s, t) => s + (t.discAmount || 0), 0),
      gallons: processed.reduce((s, t) => s + (t.fuelQuantity || 0), 0),
    }),
    [processed],
  );
  ...
  return (
    <div style={s('display:flex;flex-direction:column;gap:14px')}>
      ...
      {report?.moreRecords && (
        <div style={s('padding:10px 12px;border-radius:10px;background:color-mix(in srgb,var(--warn) 12%,transparent);border:1px solid color-mix(in srgb,var(--warn) 30%,transparent);font-size:12px;color:var(--text2)')}>
          ...
          <div>
            ...
            <div style={s('display:flex;flex-wrap:wrap;gap:12px 16px;font-size:12px;color:var(--text2)')}>
              {DISPLAY_CHECKS.map(([key, label]) => (
                <label key={key} style={s('display:flex;align-items:center;gap:6px;cursor:pointer')}>
                  <input
                    type="checkbox"
