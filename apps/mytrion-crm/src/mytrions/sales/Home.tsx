  ...
  return (
    <div className="flex flex-col gap-3.5 p-5">
      ...
      <div className="grid grid-cols-1 gap-3.5 lg:grid-cols-[1.4fr_1fr]">
        <div className="flex flex-col justify-center rounded-xs border bg-card p-5">
      ...
      <div className="rounded-xs border bg-card p-5">
        <div className="mb-3 flex items-center justify-between">
          ...
          <span className="rounded-xs border border-primary/30 bg-primary/10 px-2 py-0.5 text-[10.5px] font-bold text-primary">
          ...
          {ANNOUNCEMENTS.map((a) => {
            ...
            return (
              <button
                ...
                className="flex w-64 flex-none flex-col gap-2 rounded-xs border bg-muted/30 p-3.5 text-left hover:bg-muted/50"
              >
                <span className={`flex size-8 items-center justify-center rounded-xs ${meta.className}`}>
          ...
          {SNAPSHOT_GROUPS.map((g) => (
            <div key={g.title}>
              ...
              <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4">
                {g.cells.map((c) => (
                  <div key={c.label} className="rounded-xs border bg-muted/30 px-3 py-2.5">
      ...
      <div className="rounded-xs border bg-card p-5">
        <div className="mb-3 flex items-center justify-between">
          ...
          <div className="flex gap-1 rounded-xs border bg-muted/30 p-1">
        ...
        <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4 lg:grid-cols-7">
          {activityTiles.map((t) => (
            <div key={t.label} className="flex flex-col items-center gap-1.5 rounded-xs border bg-muted/30 px-2 py-3 text-center">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          {CALL_TO_ACTIONS.map((cta) => (
            <div key={cta.id} className="rounded-xs border bg-card p-4">
              <div className="mb-2 flex flex-wrap items-center gap-1.5">
                ...
                {cta.top ? (
                  <span className="rounded-xs border border-warn/30 bg-warn/10 px-1.5 py-0.5 text-[10px] font-bold text-warn">TOP</span>
                ) : null}
              </div>
...
function AnnouncementModal({ announcement, onClose }: { announcement: Announcement; onClose: () => void }) {
  ...
  return (
    <DetailDialog
      ...
      size="md"
      badges={
        <span className={`flex items-center gap-1.5 rounded-xs px-2 py-0.5 text-[10.5px] font-bold ${meta.className}`}>
