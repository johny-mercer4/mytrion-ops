/**
 * The viewport, as a hook. THE definition of the breakpoint ladder in TypeScript.
 *
 * It lives in `ds/_internal` rather than in `src/hooks` because `ds/` is built as a standalone
 * library (`vite.lib.config.ts`) and may not import app code — so if the ladder lived in the app,
 * `ds` would need a second copy of the four numbers and they would drift. The app re-exports this
 * from `src/hooks/useMediaQuery.ts`; that direction (app → ds) is the normal one.
 *
 * `_internal`, not exported from `ds/index.ts`: per CONVENTIONS §2 a ds COMPONENT takes props and
 * nothing else. The one sanctioned exception is `DataTable`, which cannot express its card mode in
 * CSS (see its docblock) — everything else expresses responsiveness in its stylesheet, and the
 * composition layer in `mytrions/_shared` is where a viewport becomes a prop.
 *
 * CSS cannot read these — a custom property is not legal in an `@media` condition — so the same four
 * literals are written out in the stylesheets, and `src/styles/breakpoints.test.ts` asserts the two
 * copies agree.
 *
 ...
 *   md / 900  the DENSITY line — the rail takes its collapsed 68px form, gutters go compact, inputs
 *             go to 16px. Nothing moves; things get tighter.
 *
...
function listFor(query: string): MediaQueryList | null {
  ...
  // the library build ships into. Both must resolve to "desktop" rather than throw.
...
/**
 * Subscribe to a raw media query. Prefer `useBelow` — a literal width written at a call site is
 * exactly the drift `breakpoints.test.ts` exists to stop.
 */
export function useMediaQuery(query: string): boolean {
  ...
  // Server / sandbox snapshot is `false` — i.e. the DESKTOP rendering. Never a mobile one, which in
  // a headless environment would be a layout nothing can measure and a screenshot nobody expects.
 ...
 * 640, where `(max-width: 640px)` would include it. That off-by-one is not academic: the shell used
 * to switch at `max-width: 768px` while `ds/*` guarded at `max-width: 767px`, so a viewport exactly
 * 768px wide got the mobile shell with 13px inputs and iOS auto-zoomed the page.
 */
...
/** How `DataTable` should render right now. See its docblock for why this branch is in JS. */
export type DataTableMode = 'table' | 'card';

export function useDataTableMode(): DataTableMode {
  return useBelow('sm') ? 'card' : 'table';
}
