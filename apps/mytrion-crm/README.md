...
On Render the API's `buildCommand` (in `render.yaml`) builds `apps/mytrion-crm/` too, and the Node
server serves `apps/mytrion-crm/app` under `/widget`. Same-origin means the widget's live-token streaming `fetch` needs **no CORS
