...
/**
 * Wraps tab content so a switch nudges in from the direction of travel (+1 right, -1 left, 0 no
...
export function SlideIn({ dir, children }: { dir: number; children: ReactNode }): ReactElement {
  return (
    <div style={{ ['--slide-dir' as string]: dir, animation: 'octslide .22s cubic-bezier(.32,.72,0,1)' } as CSSProperties}>
      {children}
    </div>
  );
}
