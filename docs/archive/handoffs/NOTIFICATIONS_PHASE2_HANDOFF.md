> **Status:** PARTIAL (checked 2026-08-21). Receipt poller shipped (`runReceiptPoll`). Limit
> poller, weekly statement, and money-code approval were deferred and were never built.
> `Analitika/notification_system_ultraplan.md` does not exist. `WORKING_NOTES.md` is local scratch.

# Handoff: Notification tizimi — 2-bosqich (limit/receipt/statement + approval)

> Claude Code uchun. Ishlashdan oldin kodni o'qi (`src/modules/notifications/`). Ultraplan
> yo'q. Bu hujjat NIMA qilishni, mavjud kod QAYERDA ekanini va buzib bo'lmaydigan invariantlarni beradi.
