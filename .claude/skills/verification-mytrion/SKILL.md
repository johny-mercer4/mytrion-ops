...
| Highway | **Octane parse + desk type-in** | Data Center: HTML upload parse (`highwayHtmlParser.ts`). **No Highway API / scrape.** Phase 8 findings are still typed by hand (`deskHighway.ts`). |
| iSoftPull | **Octane** (Data Center) | First-party `POST /verification/flow/isoftpull/pull`. One bureau per approved billed call. Not written onto Phase 6. Live flag default **off**. |
| Plaid Link token | **Octane** (Data Center) | `POST /verification/flow/plaid/link-token` — sandbox mint, not a Check `/get`. `/get` is deferred (no session table). |
...
- Auto-run a Data Center vendor, or fire iSoftPull / Plaid `/get` without the live flag, Confirm, spend token, and ledger row.
