...
- **Read-only is the default** (CLAUDE.md rule 7). DWH pins `default_transaction_read_only=on`; AWS MySQL pins `SET SESSION TRANSACTION READ ONLY` per connection when `AWS_MYSQL_READONLY=1`. The **real** guarantee is a read-only DB user — the session pin is defence in depth.
- These are third-party/external DBs accessed directly via `integrations/`. Anything that reads **our own tenant data** still goes through `repos/` with `tenant_id` isolation (CLAUDE.md rule 2). A DB-backed *tool* is still a `ToolManifest` dispatched through `toolDispatcher` (RBAC).
...
- Prefer also giving the app a **read-only MySQL user** (`GRANT SELECT`) — that's server-enforced
  ...
  it as a `write` tool per CLAUDE.md rule 7).
